...
- `scripts/fetch-transcript.ts` — run with `bun run scripts/fetch-transcript.ts <podcast-slug> [episode-id] [--list] [--out file]`. Fetches an existing transcript from the podcast's RSS feed via `<podcast:transcript>` tags (Podcasting 2.0). Supports VTT, SRT, JSON, and HTML (Changelog-style) formats. Outputs segments JSON. Requires `rss` URL in the podcast's `info.json` links. Use `--list` to check if transcripts exist before attempting. If available, this is faster and higher quality than running Whisper.
