...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const entries: MetadataRoute.Sitemap = [
    ...
    { url: `${BASE_URL}/podcasts`, changeFrequency: "weekly", priority: 0.7 },
  ...
  for (const podcast of podcasts) {
    ...
    for (const episode of episodes) {
      const epPath = path.join(DATA_DIR, "podcasts", podcast.slug, "episodes", `${episode.id}.json`);
      entries.push({
        url: `${BASE_URL}/podcasts/${podcast.slug}/episodes/${episode.slug}`,
